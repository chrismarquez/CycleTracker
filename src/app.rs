...
impl RocketApp for Rocket<Build> {
    async fn manage_services(self) -> Rocket<Build> {
        self.manage(HelloService::new(HelloRepository::new()))
            .manage(TrackerService::new(TrackerRepository::new().await))
    }

    fn mount_controllers(self) -> Rocket<Build> {
        self.mount_controller(HelloController::new())
...
pub async fn build_rocket() -> Rocket<Build> {
    rocket::build().manage_services().await.mount_controllers()
}
